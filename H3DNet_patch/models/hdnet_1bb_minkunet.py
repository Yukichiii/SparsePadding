...
from backbone_module_scale import MINKUNetBackbone
...
class HDNet_1bb_minkunet(nn.Module):
    r"""
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
                 input_feature_dim=0, num_proposal=128, vote_factor=1, sampling='vote_fps', with_angle=False, scale=1, pad_voxel_stride=[], pad_voxel_method='none', nearest=True, normalize_weights=True):
        super().__init__()
        ...
        self.backbone_net1 = MINKUNetBackbone(input_feature_dim=self.input_feature_dim, pad_voxel_stride=pad_voxel_stride, pad_voxel_method=pad_voxel_method, nearest=nearest, normalize_weights=normalize_weights, scale=scale)
        scale = max(scale, 2)
