...
parser.add_argument('--pad_voxel_stride', type=str, default='', help='strides for pad voxel')
parser.add_argument('--pad_voxel_method', type=str, default='', help='method for padding')
parser.add_argument('--nearest', action='store_true', help='use nearest voxel feature')
parser.add_argument('--no_normalize_weights', action='store_true', help='normalize feature or not')
parser.add_argument('--rand_seed', type=int, default=123456789, help='random seed for all random init')
...
net = Detector(num_class=DATASET_CONFIG.num_class,
               ...
               pad_voxel_stride=[int(i) for i in FLAGS.pad_voxel_stride.split(',') if i!=''], 
               pad_voxel_method=FLAGS.pad_voxel_method,
               nearest=FLAGS.nearest,
               normalize_weights=not FLAGS.no_normalize_weights)
...
def evaluate_one_epoch():
    ...
    ap_calculator_2 = APCalculator(ap_iou_thresh=0.5,
        class2type_map=DATASET_CONFIG.class2type)
    ...
    for batch_idx, batch_data_label in enumerate(TEST_DATALOADER):
        ...
        ap_calculator_2.step(batch_pred_map_cls, batch_gt_map_cls)
    ...
    log_string('-'*10 + ('iou_thresh: %f'%(0.25)) + '-'*10)
    for key in metrics_dict:
        log_string('eval %s: %f'%(key, metrics_dict[key]))

    metrics_dict2 = ap_calculator_2.compute_metrics()
    log_string('-'*10 + ('iou_thresh: %f'%(0.5)) + '-'*10)
    for key in metrics_dict2:
        log_string('eval %s: %f'%(key, metrics_dict2[key]))

    mAP_at_05 = metrics_dict2['mAP']
    mean_loss = stat_dict['loss']/float(batch_idx+1)
    return mean_loss, mAP_at_05
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, MAX_EPOCH):
        ...
        if EPOCH_CNT == 0 or EPOCH_CNT % 10 == 9: # Eval every 10 epochs
            loss, mAP_at_05 = evaluate_one_epoch()
        # Save checkpoint
        ...
        if best_mAP <= mAP_at_05:
            best_mAP = mAP_at_05
            torch.save(save_dict, os.path.join(LOG_DIR, 'best_mAP.tar'))
        print(mAP_at_05)
        print(best_mAP)
